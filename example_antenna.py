...
"""
Simple script to illustrate an array simulator.

The signal-of-interest is a 1GHz sinusoidal signal from a source on the
sky at theta=30, phi=0. The array comprises two antennas positioned along 
the x-axis, one wavelength apart, so that the SoI will be received with
half-period relative difference at the two antennas.  Additionally some
independent gaussian noise has been added to each antenna signal, as well
as a common noise signal.
"""
...
def main():
	...
	# For each antenna all the sources visible to that antenna should be
	# added to it.
	# 
	# ant0
	ant0.add_source((soi_src,n0_src,nc_src))
	# 
	# ant1
	ant1.add_source((soi_src,n1_src,nc_src))
	
	# Now request the output signals at each of the antennas.
	r0 = ant0.receiver_block().output()
	r1 = ant1.receiver_block().output()
